defmodule Change do
  ...
  def greedy(_coins, 0, given), do: {:ok, given}
  def greedy([], _target, _given), do: {:error, "cannot change"}

  def greedy(coins = [first_coin | other_coins], target, given) do
  ...
  def dynamic(_coins, 0, given, _max), do: {:ok, given}
  def dynamic([], _target, _given, _max), do: {:error, "cannot change"}

  def dynamic(_coins, _target, given, max) when length(given) >= max,
    do: {:error, "cannot change"}

  def dynamic(coins = [first_coin | other_coins], target, given, max) do
    if first_coin > target do
  ...
  def best_solution(sol1 = {status1, given1}, sol2 = {status2, given2}) do
    cond do
      status1 == :error -> sol2
      status2 == :error -> sol1
