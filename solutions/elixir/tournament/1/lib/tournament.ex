defmodule Tournament do
  ...
  def tally(input) do
      ...
      teams = score_board |> Map.keys|> Enum.sort(&(compare_teams(score_board, &1, &2)))
  ...
  defp compare_teams(scores, team1, team2) do
    ...
    if(team1_points == team2_points, do: team1 < team2, else: team1_points > team2_points)
  end

  defp update_team_score(scores, team, result) do
    case result do
      "win" -> Map.update(scores, team, {1, 1, 0, 0, 3}, fn {mp, w, d, l, p} -> {mp+1, w+1, d, l, p+3} end)
      "loss" -> Map.update(scores, team, {1, 0, 0, 1, 0}, fn {mp, w, d, l, p} -> {mp+1, w, d, l+1, p} end)
      "draw" -> Map.update(scores, team, {1, 0, 1, 0, 1}, fn {mp, w, d, l, p} -> {mp+1, w, d+1, l, p+1} end)
      _ -> scores
    end
  end
