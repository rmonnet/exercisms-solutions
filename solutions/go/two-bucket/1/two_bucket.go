...
func (s *state) fill(b int) {
	s.nsteps++
	s.history = append(s.history, s.volume)
	s.volume[b] = s.capacity[b]
}

func (s *state) empty(b int) {
	s.nsteps++
	s.history = append(s.history, s.volume)
	s.volume[b] = 0
}

func (s *state) pour(from, into int) {
	s.nsteps++
	s.history = append(s.history, s.volume)
...
func (s state) isStepAllowed() bool {
	...
	return !(s.volume[s.firstBucket] == 0 && s.volume[otherBucket] == s.capacity[otherBucket])
}
	...
	for _, path := range paths {
		for i := 0; i < 2; i++ {
			...
			if !path.isEmpty(i) {
				newPath := path
				newPath.empty(i)
				if !newPath.hasCycle() && newPath.isStepAllowed() {
...
func Solve(sizeBucketOne, sizeBucketTwo, goalAmount int, startBucket string) (string, int, int, error) {
	...
	startingPath := NewState(startingBucket, sizeBucketOne, sizeBucketTwo)

	paths := []state{startingPath}
	for i := 0; i < 1000; i++ {
