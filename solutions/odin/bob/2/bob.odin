...
is_yelling :: proc(input: string) -> bool {
	input_uc := strings.to_upper(input)
	input_lc := strings.to_lower(input)
	defer {
		delete(input_uc)
		delete(input_lc)
	}
	// If there are no letters (just numbers and symbols) that is not considered yelling.
	// In that case both Lower Case and Upper Case will be equal to the original input
	return input == input_uc && input != input_lc
}
